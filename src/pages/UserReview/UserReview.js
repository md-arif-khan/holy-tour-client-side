...
const UserReview = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/userreview?email=${user?.email}`,{
      ...
      .then((res) => res.json())
      .then((data) => setPersonalReview(data));
  }, [user?.email]);
  console.log(personalReview);
  return (
    <div>
      {
        personalReview.length>0?<div className="overflow-x-auto w-full ">
        <table className="table w-full">
          ...
          <tbody>
            {personalReview.map((review) => (
              <UserReviewItem
                personalReview={personalReview}
                setPersonalReview={setPersonalReview}
                key={review._id}
                review={review}
              ></UserReviewItem>
            ))}
